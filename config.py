...
def intro():
	msg = "Charlie - Assistente Virtual - version {} / by: Victoria Gabriella, conhecida como Senhorita Katsumi".format(version)
	print("-" * len(msg) +  "\n{}\n".format(msg)  +   "-" * len(msg))
...
conversas = {
	...
	"sim": "Lá vai, ",
	"não": "Posso te contar uma piada",
	"sim e você": "Estou bem obrigada por perguntar",
	"Qual o seu nome?" : "Me chamo Charlie, prazer",
	"Charlie": "Como posso ajudar?"
...
def verifica_nome(user_name):
	...
	if user_name.startswith("Pode me chamar de"):
    		user_name = user_name.replace("Pode me chamar de", "")
	...
	try:
		if "google" in fala:
			...
			return "abrindo facebook"
				...
				web.get(caminho_navegador).open("instagram.com.br/")
				return "abrindo instagram"
		elif "linkedin" in fala:
				web.get(caminho_navegador).open("linkedin.com/in/victoriagcosta/")
				return "abrindo seu linkedin"
		else:
